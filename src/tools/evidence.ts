...
export function buildMetroraUnknownEvidence(question: string, scope: MetroraToolScope): MetroraToolEvidence {
  return {
    ...
    scope,
    refs: [],
    coverage: { level: 'unavailable', state: 'UNSUPPORTED', label: 'Question needs a supported category', detail: 'Metrora currently covers measured spend, observed cost per call, provider quota, and controlled Bench questions.' },
    assumptions: [],
