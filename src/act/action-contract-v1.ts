...
export const ACTION_KIND_RUN_CORE_CONFORMANCE_BENCH = 'run-core-conformance-bench' as const
...
export type ApprovedActionV1 = {
  contract: ActionContractV1
  approval: ActionApprovalTokenV1
}
...
export type ActionOperationStateV1 = {
  ...
  rollback: {
    capability: 'none'
    reason: typeof ACTION_NO_ROLLBACK_REASON
  }
}

export class ActionContractError extends Error {
  constructor(
    public readonly code: 'invalid-contract' | 'invalid-approval',
    message: string,
  ) {
    super(message)
...
function parseContractShape(input: unknown): ActionContractV1 {
  ...
  if (contract.target.model !== contract.arguments.model) throw invalidContract()
  if (contract.arguments.runId !== contract.actionId) throw invalidContract()
  if (contract.timeout.operationMs !== (contract.limits.checksPlanned + 1) * contract.timeout.perRequestMs) throw invalidContract()
  const resultPair = [contract.resultIdentity.runId, contract.resultIdentity.resultDigest]
  if ((resultPair[0] === null) !== (resultPair[1] === null)) throw invalidContract()
  const evidencePair = [contract.evidenceReferences.runId, contract.evidenceReferences.resultDigest]
  if ((evidencePair[0] === null) !== (evidencePair[1] === null)) throw invalidContract()
  if (contract.approval.confirmationDigest !== null && contract.approval.confirmationDigest !== computeActionConfirmationDigest(contract.approval.proposalDigest)) {
    throw invalidContract()
  }
  return contract
}
...
export function computeActionProposalDigest(input: ActionContractV1): string {
  ...
  return sha256Json(proposalPayload(contract))
}
...
export function computeActionConfirmationDigest(proposalDigest: string): string {
  ...
  return sha256Json({
    domain: ACTION_CONFIRMATION_DIGEST_DOMAIN,
...
export function validateActionContractV1(input: unknown): ActionContractV1 {
  const contract = parseContractShape(input)
  const expectedProposalDigest = sha256Json(proposalPayload(contract))
  if (contract.approval.proposalDigest !== expectedProposalDigest) throw invalidContract()
  return contract
}
...
export function parseActionApprovalTokenV1(input: unknown): ActionApprovalTokenV1 {
  try {
    ...
    throw new ActionContractError('invalid-approval', 'approval token must contain JSON-safe values only')
  }
...
export type CreateCoreConformanceActionInput = {
  model: string
...
export function createCoreConformanceBenchAction(input: CreateCoreConformanceActionInput): ActionContractV1 {
  ...
  const operationMs = (CORE_CHECK_COUNT + 1) * perRequestMs
  const draft = {
    contractVersion: ACTION_CONTRACT_VERSION,
    schemaVersion: ACTION_SCHEMA_VERSION,
    actionId,
    ...
    target: {
      runtime: { id: 'ollama-local' as const, endpoint: OLLAMA_LOCAL_BASE_URL },
      model,
      pack: { selector: CORE_TASK_PACK_SELECTOR, packId: CORE_TASK_PACK_ID, version: CORE_TASK_PACK_VERSION, digest: CORE_TASK_PACK_V1.digest },
    },
    arguments: { model, runId: actionId },
    preconditions: {
      explicitModel: true as const,
      canonicalPack: true as const,
      runtime: 'ollama-local-only' as const,
      credentials: 'none' as const,
      shell: 'none' as const,
      arbitraryPrompt: 'rejected' as const,
      arbitraryPaths: 'rejected' as const,
      arbitraryEndpoints: 'rejected' as const,
      repositoryCodeExecution: 'none' as const,
    },
    declaredEffects: {
      network: 'loopback-only' as const,
      writes: ['metrora.act.journal.v1', 'metrora.bench-history.v1'] as ['metrora.act.journal.v1', 'metrora.bench-history.v1'],
      credentials: 'none' as const,
      shell: 'none' as const,
      repositoryCodeExecution: 'none' as const,
      promptSource: 'canonical-pack-only' as const,
      hiddenEffects: 'none' as const,
    },
    limits: {
      checksPlanned: CORE_CHECK_COUNT,
      maxConcurrentOperations: 1 as const,
      maxResponseBytes: MAX_RESPONSE_BYTES,
      maxOutputBytes: MAX_OUTPUT_BYTES,
      maxStreamChunks: MAX_STREAM_CHUNKS,
      maxStreamEvents: MAX_STREAM_EVENTS,
    },
    timeout: { perRequestMs, operationMs },
    cancellation: {
      policy: 'abort-signal-propagates' as const,
      lateResults: 'discard' as const,
      terminalStatePrecedence: 'cancelled-or-timeout' as const,
    },
    approval: {
      required: 'explicit-user-confirmation' as const,
      proposalDigest: EMPTY_DIGEST,
  ...
  } satisfies Omit<ActionContractV1, 'approval'> & { approval: ActionContractV1['approval'] }
  const proposalDigest = computeActionProposalDigest(draft)
  return validateActionContractV1({
    ...draft,
    approval: { ...draft.approval, proposalDigest },
  })
}

function approvalSigningPayload(token: Omit<ActionApprovalTokenV1, 'proof'>): string {
...
export class TrustedActionAuthorityV1 {
  ...
  constructor(secret?: Uint8Array) {
    if (secret !== undefined && secret.byteLength < 32) throw new Error('trusted action authority secret must be at least 32 bytes')
    this.secret = Buffer.from(secret ?? randomBytes(32))
  }

  /**
   * The host process calls this only after receiving a real user confirmation.
   * The renderer/LLM receives no signing key and this method is not part of the
   * bounded bridge surface.
   */
  issueApprovalAfterTrustedUserConfirmation(contractInput: ActionContractV1): ApprovedActionV1 {
    ...
    if (contract.approval.confirmationDigest !== null) throw new ActionContractError('invalid-approval', 'action already contains a confirmation digest')
    ...
    const unsigned = {
      tokenVersion: ACTION_APPROVAL_TOKEN_VERSION,
      actionId: contract.actionId,
      proposalDigest,
      confirmationDigest,
      issuedAt: new Date().toISOString(),
      authority: ACTION_APPROVAL_AUTHORITY,
    } satisfies Omit<ActionApprovalTokenV1, 'proof'>
    const proof = createHmac('sha256', this.secret).update(approvalSigningPayload(unsigned), 'utf8').digest('hex')
    const approval = parseActionApprovalTokenV1({ ...unsigned, proof })
    const approvedContract = validateActionContractV1({
      ...contract,
      approval: { ...contract.approval, confirmationDigest },
    })
  ...
  verifyApprovedAction(input: unknown): ApprovedActionV1 {
    ...
    if (
      approval.actionId !== contract.actionId
      || approval.proposalDigest !== expectedProposalDigest
      || approval.confirmationDigest !== expectedConfirmationDigest
      || contract.approval.confirmationDigest !== expectedConfirmationDigest
    ) {
      throw new ActionContractError('invalid-approval', 'approval is not bound to this exact action proposal')
    }
    const expectedProof = createHmac('sha256', this.secret).update(approvalSigningPayload({
      tokenVersion: approval.tokenVersion,
    ...
    if (expectedBytes.length !== actualBytes.length || !timingSafeEqual(expectedBytes, actualBytes)) {
      throw new ActionContractError('invalid-approval', 'approval proof was not issued by the trusted process')
    }
    return { contract, approval }
  }
}

export function coreConformancePackIdentity(): { selector: typeof CORE_TASK_PACK_SELECTOR; packId: typeof CORE_TASK_PACK_ID; version: typeof CORE_TASK_PACK_VERSION; checks: typeof CORE_CHECK_COUNT; digest: string } {
  return {
    selector: CORE_TASK_PACK_SELECTOR,
    packId: CORE_TASK_PACK_ID,
    version: CORE_TASK_PACK_VERSION,
    checks: CORE_CHECK_COUNT,
    digest: CORE_TASK_PACK_V1.digest,
  }
}
