...
The additive Companion Capacity V1 projection exposes this same canonical snapshot set to an authenticated Android companion through Desktop; it does not add a second quota engine, provider endpoints, credentials or mobile-side allowance calculation. The projection does not invent quota for unsupported providers, conflate quota with local budgets, change usage accounting or pricing, or change the parked native macOS parity work. Additional providers must enter through the same source hierarchy and `ProviderQuotaSnapshot` contract rather than introducing provider-specific UI authority.
