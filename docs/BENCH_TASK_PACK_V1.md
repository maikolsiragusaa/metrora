...
Core Compatibility remains a Metrora Bench/CLI contract. The OpenCode Desktop
integration does not expose the Bench registry or start a Compatibility run
from a coding prompt. Its one custom tool is limited to the canonical Metrora
usage snapshot, so this pack remains separate from OpenCode session behavior.
