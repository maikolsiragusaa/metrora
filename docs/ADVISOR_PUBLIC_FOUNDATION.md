...
For a factual/tool turn, the first bounded model call receives same-scope conversation, UI context, and the fixed read-only tool definitions. It may answer directly or select typed tools; Metrora executes canonical reads locally and appends the exact call ID and bounded result to the same provider-neutral turn ledger before asking the selected model to continue. Provider adapters project that semantic ledger into the provider’s native assistant-tool/result form when native calls are proven; models without proven native calls use only the bounded structured-text fallback. Ordinary chat can finish after the first call. Ollama uses bounded NDJSON parsing; LM Studio uses bounded OpenAI-compatible SSE parsing. A discovered model is not treated as verified Advisor tool support: each discovered model receives a session-local `ModelCapabilityProfileV1` with conversational availability and streaming support facts, while tool support remains `unknown` until advertised capability or a bounded conformance check establishes more. Deterministic comprehension remains a fallback hint, same-scope history is bounded, UI context is referential only, and action language stays proposal-only.
...
The deterministic local runtime remains an internal recovery primitive when no usable model is connected; it is not presented as a second conversational mode. A failed selected model produces a concise runtime failure, with successfully retrieved facts available through Sources/Details. Local BYOK supports OpenAI, Anthropic, Gemini, OpenRouter, and OpenCode Zen through fixed provider descriptors, protected local credentials, bounded responses, and direct provider traffic from Electron main; Metrora is not an inference proxy. Conversation state remains client-managed. OpenAI Responses requests explicitly send `store: false` and omit conversation, previous-response, and background state. Anthropic and Gemini are stateless from Metrora’s perspective. OpenRouter uses its direct Chat Completions surface. OpenCode Zen resolves the documented protocol per model: Responses, Anthropic Messages, OpenAI-compatible Chat Completions, or Gemini `generateContent`; a model with no reviewed mapping is unsupported rather than guessed. Each runtime uses one bounded model step at a time, with native Tool-result continuation where proven and the structured fallback only where native calls are not proven; direct ordinary answers do not require a second model call. Arbitrary OpenAI-compatible endpoints and managed inference remain out of scope.
...
Conversation history is session-local in the renderer and is not synced or persisted by this foundation. The initial chat call receives only bounded same-scope conversation, the current question, the deterministic semantic fallback hint, the deterministic guard contract, bounded UI context, and tool schemas. If canonical reads occur, the same provider-neutral ledger is replayed for continuation with exact assistant Tool calls and Tool results when native semantics are proven; the bounded flattened representation is used only for an explicitly unproven fallback model. A hosted BYOK runtime shares only that minimum content-minimal context after explicit consent. Raw source content, paths, secrets, and hidden reasoning are excluded from the public contract. Hosted lifecycle events exposed to the renderer contain only safe status metadata and raw provider bodies/deltas stay in the main-process boundary; bounded normalized responses and read-only tool-call arguments are transient inputs excluded from logs, evidence, mobile projection, and sync.
