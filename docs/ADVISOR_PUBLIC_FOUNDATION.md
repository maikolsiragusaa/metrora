...
The deterministic guard owns social/action/boundary decisions, authorization, canonical scope, privacy, the fixed read-only allowlist, and cancellation/bounds. It does not veto the semantic family of a normal safe investigation. The existing deterministic EN/IT classifier remains useful as an offline plan, a bounded planning hint, and a fallback when model planning is malformed or unavailable. For a material investigation, Metrora sends a bounded planning call; the validated model plan owns the supported question family, evidence domains, bounded read-tool selection, presentation intent, and requested detail depth. Metrora then executes only canonical read-only evidence requests, merges same-scope evidence, and sends a fresh independent synthesis call. Planning contains no factual answer. Synthesis selects only Metrora-owned `AdvisorVerifiedClaimAtomV1` IDs and presentation kinds; Metrora verifies the atom kind/path/value/scope relationship and renders the material factual clauses in canonical EN/IT copy. Chart/table values are resolved by Metrora, never supplied by model prose. The connected path is labelled “model-assisted investigation”.
...
| OpenRouter | `GET https://openrouter.ai/api/v1/models?output_modalities=text` | Chat Completions `/api/v1/chat/completions` | `supported_parameters` exposes tool support; text-only models use a limited deterministic-tool path |
...
Discovery is not compatibility. A returned model receives a bounded state (`discovered`, `unverified`, `verified`, `limited`, `unsupported`, or `failed-conformance`) plus conversational, streaming and tool-call capability facts. The UI can select only models that are not `unsupported` or `failed-conformance`; unknown tool support never causes provider-native tools to be sent. The seven existing `AdvisorToolV1` identities, read-only authority, content-minimal evidence projection and fresh planning/synthesis boundary remain unchanged.

Credentials remain in the existing Electron `safeStorage` file and are read only by the main-process adapter. Provider switching invalidates active requests, stale model/consent state and in-flight probe results. Evidence-sharing consent is explicit, starts unchecked, and resets on provider/model/authority changes. No credential, prompt, response, raw provider body, tool arguments or arbitrary endpoint crosses the renderer boundary.
...
Conversation history is session-local in the renderer and is not synced or persisted by this foundation. CALL A receives only bounded same-scope conversation, the current question, the deterministic semantic fallback hint, the deterministic guard contract, and tool schemas. After canonical reads, CALL B receives a fresh bounded context containing the question, same-scope conversation, verified scope, validated plan, selectable typed claim atoms, and final content-minimal evidence; tool results are not replayed as provider-native messages. A hosted BYOK runtime shares only that minimum content-minimal context after explicit consent. Raw source content, prompts, paths, secrets, and hidden reasoning are excluded from the public contract. Hosted lifecycle events exposed to the renderer contain only safe status metadata; provider text, tool-call arguments, and raw deltas stay in the main-process boundary.
