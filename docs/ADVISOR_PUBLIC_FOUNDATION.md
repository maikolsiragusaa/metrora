...
Deterministic guards own authorization, canonical scope, privacy, the fixed read-only allowlist, cancellation, and bounds at their respective boundaries. For a capable runtime, the deterministic EN/IT comprehension result is only a bounded fallback hint; the model receives conversation and optional typed tools first. It may answer directly or request bounded reads. Metrora executes only canonical read-only requests, merges same-scope evidence, and can return to the model for a second bounded read round when the first result shows that another authorized fact is needed. The loop is capped at two read rounds, four total Tool calls and four calls per round, followed by a no-tool final pass. Synthesis selects only Metrora-owned AdvisorVerifiedClaimAtomV1 IDs for factual clauses; a separate bounded narrative may add evidence-grounded interpretation, recommendation or explicit limits without inventing facts. Ordinary chat does not need an evidence bundle, and chart/table values are resolved by Metrora rather than model prose.
...
- llama-server: loopback `http://127.0.0.1:<port>/health` and `/v1/models` for discovery, then `/v1/chat/completions`; the port is validated from `1` through `65535`, defaults to `8080`, and only existing local servers are accepted.

The LM Studio adapter reads language-model identifiers factually from the local `models` response and ignores embedding models. The default local server does not require a token; configurations that require authentication are unavailable in this Community V1 path because Metrora does not add credential management. Tokens are never logged, stored in renderer state, or synchronized. The endpoint and protocol remain fixed; only the llama-server loopback port is renderer-configurable within the bounded range. Arbitrary URLs, LAN binding, remote OpenAI-compatible servers, and cloud fallback are not accepted.
...
Conversation history is session-local in the renderer and is not synced or persisted by this foundation. A model call receives only bounded same-scope conversation, the current question, a minimal deterministic fallback hint, bounded UI context, and the fixed typed Tool definitions needed for the selected capability. If canonical reads occur, later calls receive only the verified scope and bounded content-minimal evidence projection; tool results are not replayed as provider-native messages. A hosted BYOK runtime shares only that minimum context after explicit consent. Raw source content, paths, secrets, hidden prompts, guard contracts, private scratchpads and raw provider payloads are excluded from the public contract. Hosted lifecycle events exposed to the renderer contain only safe status metadata; bounded normalized responses and read-only tool-call arguments are transient inputs excluded from logs, evidence, mobile projection, and sync.
...
Synthetic conformance tests cover the bounded loopback endpoint and configurable-port validation, incremental NDJSON, cancellation, bounded errors, nullable evidence, mixed quota freshness, unavailable-provider privacy, multi-tool aggregation, deterministic fact authority, chat-first generic turns, bounded UI context/history, action safety, and the session-local welcome surface.
