...
The deterministic guard owns facts, arithmetic, coverage, currency formatting, provider-quota freshness, scope, authorization, and rich presentation values. For a material investigation, the guarded question enters a bounded model-planning call. Metrora validates the returned `advisor-planning-draft-v1`, executes only canonical read-only evidence requests, merges same-scope evidence, and sends a fresh independent synthesis call. Planning contains no factual answer; synthesis is accepted only through the verified `advisor-synthesis-draft-v1` claim graph. Chart/table values are resolved by Metrora, never supplied by model prose. Unverified model prose is not displayed as an answer, and the connected path is labelled “model-assisted investigation”.
...
When a connected model supplies a synthesis draft, `AdvisorSynthesisDraftV1` conclusion/why/details blocks use `{ text, claimIds }`; every material block must point to verified claims. Each claim must reference an exact Metrora evidence item and deterministic evidence path. Metrora rejects orphan prose, unsupported causal/forecast/recommendation claims, qualitative claims without evidence, invented facts, and sensitive claims. `AdvisorPresentationBlockV1` values are then built from canonical evidence for metric cards, native SVG charts, comparison tables, quota cards, Bench summaries, warnings, and evidence disclosure. Missing values remain unavailable rather than being zero-filled.
