...
- [ ] In Bench → Performance, confirm the managed official **CPU** `llama-bench` component starts as Not installed, offers an explicit Install CPU component action with progress/cancel/retry, and after installation is used automatically; choosing an existing executable remains available for accelerated builds. Confirm the setup shows bounded repetitions/prompt/decode/batch/ubatch/GPU/Flash-Attention settings and no arbitrary command-line field.
