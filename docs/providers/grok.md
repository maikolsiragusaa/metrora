...
When Grok explicitly supplies valid `reasoningTokens`, it is a factual subset of the reported `outputTokens`; Metrora preserves the provider-reported output unchanged, bounds reasoning to that output, and marks the call `reasoningSemantics: aggregate-output`. An absent or malformed reasoning field uses the numeric zero bucket with `reasoningSemantics: unavailable`, not factual zero evidence. Sessions with heterogeneous reasoning evidence use `mixed`. Reasoning is never added again to generated or billable output. `modelUsage` is used only to choose one session attribution model; top-level totals are never split by model.
