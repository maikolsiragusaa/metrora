...
function PresentationBlocks({ blocks }: { blocks: AdvisorPresentationBlockV1[] }) {
  return <div className="advisor-presentation">{blocks.map((block, index) => {
    ...
    if (block.kind === 'bench-summary') return <section className="advisor-presentation-block" key={index}><div className="advisor-presentation-head"><h4>{block.title}</h4><span>{block.scopeLabel} · {block.periodLabel}</span></div><p className="advisor-presentation-summary">{block.summary}</p>{block.run ? <div className="advisor-bench-summary"><strong>{block.run.model.selected}</strong><span>{block.run.aggregate.passed}/{block.run.aggregate.planned} planned tasks passed</span><span>{block.run.aggregate.scoreValue === null ? 'Score unavailable' : block.run.aggregate.scoreDenominator === null ? (block.run.aggregate.scoreValue * 100).toFixed(1) + '% score' : (block.run.aggregate.scoreValue * 100).toFixed(1) + '% of ' + block.run.aggregate.scoreDenominator + ' scored checks'}</span><span>Status · {block.run.status}</span></div> : <p className="advisor-muted-line">No completed controlled run is available.</p>}</section>
