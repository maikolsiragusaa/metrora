...
describe('PullRequests', () => {
  ...
  it('closes an open expansion when the period changes the PR set', async () => {
    const changed: PrPayload = { ...SAMPLE, rows: [SAMPLE.rows[0]!] }
  ...
  it('marks an approximate (legacy) row with a ~ prefix and a tooltip', async () => {
    const approxPayload: PrPayload = {
      rows: [{ url: 'https://github.com/getagentseal/codeburn/pull/900', label: 'getagentseal/codeburn#900', cost: 12.5, savingsUSD: 0, sessions: 1, calls: 30, firstStarted: '2026-07-10T10:00:00Z', lastEnded: '2026-07-10T11:00:00Z', approx: true }],
      distinctCost: 12.5, distinctSessions: 1, attributedCost: 12.5, unattributedCost: 0,
    }
  ...
  it('renders the old-CLI by-reference footer without NaN and never claims summable', async () => {
    const oldPayload: PrPayload = {
      rows: [{ url: 'https://github.com/getagentseal/codeburn/pull/500', label: 'getagentseal/codeburn#500', cost: 120.4, savingsUSD: 0, sessions: 2, calls: 300, firstStarted: '2026-06-01T10:00:00Z', lastEnded: '2026-06-02T12:00:00Z' }],
      distinctCost: 120.4, distinctSessions: 2,
    }
  ...
  it('renders the complete PR list without an opaque Other row', async () => {
    const manyRows = Array.from({ length: 32 }, (_, index) => ({ ...SAMPLE.rows[0]!, url: `https://github.com/getagentseal/codeburn/pull/${800 + index}`, label: `getagentseal/codeburn#${800 + index}` }))
    getOverview.mockResolvedValue(makePayload({ ...SAMPLE, rows: manyRows, attributedCost: manyRows.reduce((sum, row) => sum + row.cost, 0) }))
    render(<PullRequests period="lifetime" provider="all" />)
