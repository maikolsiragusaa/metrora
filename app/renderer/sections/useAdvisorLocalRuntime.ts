...
function createLocalRuntime(runtime: AdvisorLocalRuntimeId, model: string): LocalRuntimeInstance {
  if (runtime === 'lmstudio') return new LMStudioAdvisorRuntime({ model, availability: 'ready' })
  if (runtime === 'llama-server') return new LlamaServerAdvisorRuntime({ model, availability: 'ready' })
...
export function useAdvisorLocalRuntime(): {
  ...
  const setLocalModel = useCallback((model: string) => {
    ...
    setLocalRuntime(createLocalRuntime(runtimeId, model))
  }, [runtimeId])

  return { runtimeId, setRuntimeId, runtimeModel, setRuntimeModel, runtimeState, localRuntime, checkLocalRuntime, setLocalModel }
}
