...
function durableAccounting(data: MenubarPayload): DurableModelAccounting {
  ...
  const rows = data.current.topModels.map(model => ({ name: model.name, cost: model.cost, savingsUSD: model.savingsUSD, calls: model.calls }))
...
export function Models({ period, provider, range = null, refreshToken = 0, onNavigate, ready = true }: {
  period: Period
  provider: string
  ...
  return <>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'flex-start' }}>
      ...
      {lens !== 'audit' && <button type="button" className="btn btn-s" onClick={() => onNavigate?.('compare')}>Compare…</button>}
    </div>
    {lens === 'audit'
      ? <AuditLens period={period} provider={provider} range={range} refreshToken={refreshToken} ready={ready} />
      : <ModelsUsage period={period} provider={provider} range={range} byTask={lens === 'task'} refreshToken={refreshToken} onAddAlias={onAddAlias} ready={ready} />}
  </>
}

function ModelsUsage({ period, provider, range, byTask, refreshToken, onAddAlias, ready }: {
  period: Period
  provider: string
  range: DateRange | null
  byTask: boolean
  ...
  if (byTask) {
    ...
    return <>
      ...
      <Panel className="scroll-x">
        ...
        {report.data.length ? <ModelsTable rows={report.data} byTask onAddAlias={onAddAlias} /> : <EmptyNote>No detailed task data in this range yet.</EmptyNote>}
      </Panel>
    </>
  }
  if (!durable.data) {
  ...
  return <>
    ...
    <Panel className="scroll-x">
      ...
      {hasAccountingValue(accounting) ? <DurableModelsTable accounting={accounting} /> : <EmptyNote>No model usage in this range yet.</EmptyNote>}
    </Panel>
    <Panel className="scroll-x">
      <div style={{ padding: '12px 14px 4px' }}>
...
function DurableModelsTable({ accounting }: { accounting: DurableModelAccounting }) {
  ...
  if (accounting.gap.cost > 0.000001 || accounting.gap.calls > 0 || accounting.gap.savingsUSD > 0.000001) rows.push({ name: 'Other models', ...accounting.gap })
  rows.sort((left, right) => (right.cost - left.cost) || (right.calls - left.calls))
  return <table aria-label="Model totals">
    <thead><tr><th>Model</th><th>Calls</th><th>Cost</th><th>Saved</th></tr></thead>
    <tbody>{rows.map((model, index) => <tr key={`${model.name}-${index}`}>
      <td><span className="mdot" style={{ display: 'inline-block', background: seriesColorForModel(model.name), marginRight: 8 }} />{model.name}</td>
      <td>{fmtInt(model.calls)}</td><td>{formatUsd(model.cost)}</td><td className={model.savingsUSD > 0 ? 'pos' : undefined}>{formatUsd(model.savingsUSD)}</td>
    </tr>)}</tbody>
  </table>
}
...
function AuditLens({ period, provider, range, refreshToken, ready }: { period: Period; provider: string; range: DateRange | null; refreshToken: number; ready: boolean }) {
  ...
  return <>{report.error && <StaleBanner error={report.error} />}<Panel className="scroll-x">{report.data.length ? <AuditTable rows={report.data} /> : <EmptyNote>No model usage to audit in this range yet.</EmptyNote>}</Panel></>
}
function AuditTable({ rows }: { rows: AuditRow[] }) {
  return <table className="audit-table"><thead><tr><th>Model</th><th>Calls</th><th>Input</th><th>Output</th><th>Reasoning</th><th>Norm out</th><th>Cache wr</th><th>Cache rd</th><th>Cost</th></tr></thead><tbody>{rows.map((row, i) => <AuditTableRow key={`${row.provider}-${row.model}-${i}`} row={row} />)}</tbody></table>
}
function AuditTableRow({ row }: { row: AuditRow }) {
  const estimated = auditEstimated(row)
  return <tr>
    <td title={row.model}><span className="mdot" style={{ display: 'inline-block', background: seriesColorForModel(row.modelDisplayName || row.model), marginRight: 8 }} />{row.modelDisplayName}</td>
    <td>{fmtInt(row.calls)}</td><td>{formatCompact(row.raw.inputTokens)}</td><td>{formatCompact(row.raw.outputTokens)}</td><td>{formatCompact(row.raw.reasoningTokens)}</td><td>{formatCompact(row.displayed.outputTokens)}</td><td>{formatCompact(row.displayed.cacheWriteTokens)}</td><td>{formatCompact(row.displayed.cacheReadTokens)}</td>
    <td>{formatUsd(row.attributedCostUSD)}{estimated ? <span className="est" title="Cost is estimated (no live pricing or derived rate)"> est</span> : null}</td>
  </tr>
}
function ModelsTable({ rows, byTask, onAddAlias }: { rows: ModelReportRow[]; byTask: boolean; onAddAlias: () => void }) {
  ...
  return <table><thead><tr><th>Model</th><th>Calls</th><th>Input</th><th>Output</th><th>Cache read</th><th>Cost</th><th>Saved</th></tr></thead><tbody>{rows.map((row, i) => <ModelTableRow key={`${row.provider}-${row.model}-${i}`} row={row} onAddAlias={onAddAlias} />)}</tbody></table>
}
function ModelsByTaskTable({ rows, onAddAlias }: { rows: ModelReportRow[]; onAddAlias: () => void }) {
  const groups = groupTaskRows(rows)
  return <table className="models-by-task"><thead><tr><th>Task</th><th>Calls</th><th>Input</th><th>Output</th><th>Cache read</th><th>Cost</th><th>Saved</th></tr></thead>{groups.map(group => <tbody className="model-task-group" key={`${group.provider}-${group.model}`}><ModelGroupRow rows={group.rows} onAddAlias={onAddAlias} />{group.rows.map((row, i) => <ModelTaskRow key={`${row.category ?? 'all'}-${i}`} row={row} />)}</tbody>)}</table>
}
function ModelTableRow({ row, onAddAlias }: { row: ModelReportRow; onAddAlias: () => void }) {
  ...
  const dotStyle = { display: 'inline-block', background: seriesColorForModel(row.modelDisplayName || row.model), marginRight: 8 }
  return <tr>
    <td title={row.model}><span className="mdot" style={dotStyle} />{row.modelDisplayName}<span style={{ ...providerTagStyle, display: 'block', marginTop: 2, paddingLeft: 16 }}>{row.providerDisplayName}</span><span style={{ ...providerTagStyle, display: 'block', marginTop: 2, paddingLeft: 16 }} title={pricing.title}>{pricing.label}</span>{pricing.showAlias ? <button type="button" className="alias" onClick={onAddAlias}>add alias ›</button> : null}</td>
    <td>{fmtInt(row.calls)}</td><td>{formatCompact(row.inputTokens)}</td><td>{formatCompact(row.outputTokens)}</td><td>{formatCompact(row.cacheReadTokens)}</td>
    <td className={pricing.muteCost ? 'dim' : undefined} title={pricing.title} aria-label={pricing.costMode === 'partial' ? `Priced portion ${costValue}` : pricing.costMode === 'unavailable' ? 'Cost unavailable' : `Cost ${costValue}`}>{costValue}</td>
...
function ModelGroupRow({ rows, onAddAlias }: { rows: ModelReportRow[]; onAddAlias: () => void }) {
  ...
  return <tr className="model-group-row">
    <td title={model.model}><span className="model-group-lead"><span className="mdot" style={{ background: seriesColorForModel(model.modelDisplayName || model.model) }} /><span style={{ display: 'flex', flexDirection: 'column', gap: 1 }}><span className="model-group-name">{model.modelDisplayName}</span><span style={providerTagStyle}>{model.providerDisplayName}</span><span style={providerTagStyle} title={pricing.title}>{pricing.label}</span></span>{pricing.showAlias ? <button type="button" className="alias" onClick={onAddAlias}>add alias ›</button> : null}</span></td>
    <td>{fmtInt(calls)}</td><td aria-label="No aggregate input" /><td aria-label="No aggregate output" /><td aria-label="No aggregate cache read" />
    <td className={pricing.muteCost ? 'dim' : undefined} title={pricing.title} aria-label={pricing.costMode === 'partial' ? `Priced portion ${costValue}` : pricing.costMode === 'unavailable' ? 'Cost unavailable' : `Cost ${costValue}`}>{costValue}</td><td className={savingsUSD > 0 ? 'pos' : undefined}>{formatUsd(savingsUSD)}</td>
  </tr>
}
function ModelTaskRow({ row }: { row: ModelReportRow }) {
  const pricing = modelPricingPresentation(row.pricing, row.calls)
  const costValue = pricing.costMode === 'unavailable' ? '—' : formatUsd(row.costUSD)
  return <tr className="model-task-row"><td>{row.category ?? 'general'}</td><td>{fmtInt(row.calls)}</td><td>{formatCompact(row.inputTokens)}</td><td>{formatCompact(row.outputTokens)}</td><td>{formatCompact(row.cacheReadTokens)}</td><td className={pricing.muteCost ? 'dim' : undefined} title={pricing.title} aria-label={pricing.costMode === 'partial' ? `Priced portion ${costValue}` : pricing.costMode === 'unavailable' ? 'Cost unavailable' : `Cost ${costValue}`}>{costValue}</td><td className={row.savingsUSD > 0 ? 'pos' : undefined}>{formatUsd(row.savingsUSD)}</td></tr>
}
