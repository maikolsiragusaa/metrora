...
function modelStateLabel(value: HarnessRuntimeModelState): string { return stateLabel(value) }
  ...
  return (
    <div className="harness-v3-runtime-anchor" ref={rootRef}>
      <button type="button" className="harness-v3-runtime-trigger" aria-label={configureOpen ? 'Close runtime' : 'Configure runtime'} aria-haspopup="dialog" aria-expanded={configureOpen} aria-controls={configureOpen ? 'harness-runtime-config' : undefined} onClick={onToggleConfigure}>
        ...
        <span className={'harness-v3-runtime-trigger-status ' + runtimeStatusKind}>{runtimeState.status === 'unavailable' && runtimeChoice !== 'hosted' ? 'Offline evidence fallback' : runtimeAvailability}</span>
      ...
      {configureOpen ? <div className="harness-v3-runtime-popover" id="harness-runtime-config" role="dialog" aria-label="Harness runtime configuration">
        ...
        {runtimeChoice === 'hosted' ? <div className="harness-v3-runtime-controls">
          ...
          <div className="harness-v3-runtime-state-grid" aria-label="Hosted runtime state"><span data-state="credential">Credential: {credentialStateLabel(hostedProbe.credentialState)}</span><span data-state="reachability">Reachability: {reachabilityLabel(hostedProbe.reachability)}</span><span data-state="model">Model: {modelStateLabel(hostedModelState)}</span><span data-state="tool-call">Tool calls: {stateLabel(selectedHostedModel?.capabilities?.toolCall ?? 'unknown')}</span></div>
