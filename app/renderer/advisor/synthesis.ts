...
function contributionSubjectCandidate(value: string): string | null {
  ...
  while (words.length && CONTRIBUTION_ENTITY_STOP_WORDS.has(words[words.length - 1]!)) {
    // Single-letter identifiers such as "Project A" are valid canonical
    // subjects, even though "a" is also an article in ordinary prose.
...
/**
 * Extracts only bounded entity-shaped mentions near contribution language.
 * It is intentionally conservative: an unrecognised candidate causes the
 * narrative to be dropped instead of being guessed as a canonical subject.
 */
...
function canonicalContributionAtoms(evidence: AdvisorEvidence): AdvisorVerifiedClaimAtomV1[] {
  return buildAdvisorVerifiedClaimAtoms(evidence)
    .filter(isCanonicalContributionAtom)
    .filter(atom => verifyAdvisorVerifiedClaimAtom(atom, evidence))
}
...
function isStrongContributionLanguage(value: string): boolean {
  ...
  return /\b(?:main|primary|top|leading|highest|largest|biggest|first|number\s+one|most\s+(?:expensive|costly)|ranking|ranked|rank|classifica|classificato|classificazione|ordinamento|ordine|principale|maggiore|primo|prima|in\s+testa|piu\s+(?:costoso|costosa|costosi|costose))\b/u.test(normalized)
}
...
function contributionCategoriesSupported(value: string, atoms: AdvisorVerifiedClaimAtomV1[]): boolean {
  ...
  return categories.every(([pattern, group]) => !pattern.test(normalized) || atoms.some(atom => contributionGroup(atom) === group))
}
...
function subjectRelevantCanonicalAtoms(value: string, evidence: AdvisorEvidence, atoms: AdvisorVerifiedClaimAtomV1[]): AdvisorVerifiedClaimAtomV1[] | null {
  ...
  if (unknownCandidate) return null
  // A contribution sentence without an explicit entity is allowed to refer to
  // the selected canonical breakdown. Explicit candidates must match it.
  if (candidates.length && !namedSubjects.length) return null
...
function selectedModelPlaceholderSupported(value: string, evidence: AdvisorEvidence, atoms: AdvisorVerifiedClaimAtomV1[]): boolean {
  const normalized = normalizedPhrase(value)
  if (!/\b(?:selected\s+model|modello\s+selezionato)\b/u.test(normalized)) return true
  const selectedModel = evidence.scope.model ?? evidence.modelEfficiency?.selectedModel ?? null
...
function contributionNarrativeSupported(value: string, evidence: AdvisorEvidence, relevantAtoms: AdvisorVerifiedClaimAtomV1[]): boolean {
  ...
  if (!isStrongContributionLanguage(safe)) return true
  // Rank is proved only by a strict maximum in a canonical same-kind cost
  // breakdown. Ties, single-row slices, and missing costs fail closed.
  const allAtoms = canonicalContributionAtoms(evidence)
  return subjectAtoms.some(atom => rowIsStrictlyTop(atom, allAtoms))
}
 ...
 * stronger ranking language additionally needs a strict canonical ordering.
 */
