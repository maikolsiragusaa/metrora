...
describe('desktop navigation authority', () => {
  ...
  it('keeps Harness in Analyze and Capacity in Control without exposing Capacity as a shortcut', () => {
    ...
    expect(DESKTOP_NAVIGATION_ORDER).toEqual([
      'overview', 'sessions', 'pullRequests', 'spend', 'optimize', 'models', 'compare', 'harness', 'bench', 'plans', 'workspace', 'settings',
    ])
    expect(DESKTOP_NAVIGATION_ITEMS.optimize.label).toBe('Insights')
    expect(DESKTOP_NAVIGATION_ITEMS.harness.label).toBe('Harness')
