...
describe('desktop navigation authority', () => {
  ...
  it('keeps OpenCode in Analyze and Capacity in Control without exposing Capacity as a shortcut', () => {
    expect(DESKTOP_NAVIGATION_GROUPS).toEqual([
      ...
      { id: 'analyze', label: 'Analyze', placement: 'primary', sections: ['spend', 'optimize', 'models', 'compare', 'opencode', 'bench'] },
    ...
    expect(DESKTOP_NAVIGATION_ORDER).toEqual([
      'overview', 'sessions', 'pullRequests', 'spend', 'optimize', 'models', 'compare', 'opencode', 'bench', 'plans', 'workspace', 'settings',
    ])
    expect(DESKTOP_NAVIGATION_ITEMS.optimize.label).toBe('Insights')
    expect(DESKTOP_NAVIGATION_ITEMS.opencode.label).toBe('OpenCode')
