...
describe('desktop navigation authority', () => {
  ...
  it('exposes the control-center destinations while retaining internal routes', () => {
    ...
    expect(DESKTOP_NAVIGATION_ITEMS.bench.title).toBe('Bench')
    expect(DESKTOP_NAVIGATION_ITEMS.bench.label).toBe('Bench')
    expect(DESKTOP_NAVIGATION_ITEMS.bench.id).toBe('bench')
    expect(DESKTOP_NAVIGATION_ITEMS.bench.shortcut).toBe('5')
