...
import { legacyExternalEscalationMessageType } from '../src/compat/legacy-migration-identifiers.js'
...
describe('incremental session cache', () => {
  ...
  it('folds external escalation iteration cost into the session total', async () => {
    ...
    line.message.usage['iterations'] = [
      ...
      { type: legacyExternalEscalationMessageType(), model: 'claude-opus-4-20250514', input_tokens: 500_000, output_tokens: 50_000 },
    ]
    const withEscalation = await transcript([JSON.stringify(line) + '\n'])
    const escalationTotal = (await computeSessionUsage(emptyCache(SID), withEscalation)).cache.costUSD

    expect(plainCost).toBeGreaterThan(0)
    expect(escalationTotal).toBeGreaterThan(plainCost)

    // Streaming rewrite of the same message id replaces, not adds: the fold
    // stays stable when the escalation-carrying line is written twice.
    ...
    expect(rewrittenTotal).toBeCloseTo(escalationTotal, 10)
  })
